...
{
    public $pluralTitle = 'Interior Designer Management';
    public $singleTitle = 'Interior Designer ';
    public $cannotDelete = array();
    public function actionCreate(){
        try {
            $model = new InteriorDesign('create');
            if (isset($_POST['InteriorDesign'])) {
                $model->attributes = $_POST['InteriorDesign'];
                if($model->save())
				{
                    $model->saveImage('logo');
                    $model->saveImage('present_image');
    ...
    public function actionIndex() {
        try {
            $model=new InteriorDesign('search');
            $model->unsetAttributes();  // clear any default values
            if(isset($_GET['InteriorDesign']))
                $model->attributes=$_GET['InteriorDesign'];
    ...
    public function actionUpdate($id) {
        ...
        $this->render('update',array(
            'model' => $model,
            'actions' => $this->listActionsCanAccess,
            'title_name' => $model->interior_name        ));
    }
	...
	*/
	...
	{
		$deleteItems = $_POST['interior-design-grid_c0'];
		$shouldDelete = array_diff($deleteItems, $this->cannotDelete);

		if (!empty($shouldDelete))
		{
            if(is_array($shouldDelete) && count($shouldDelete)>0){
                $allData = InteriorDesign::model()->findAllByAttributes(array("id"=>$shouldDelete));    
                foreach($allData as $model){
                    $model->removeImage(array('logo','present_image'), true);
                    $model->delete();
                }
            }
			//InteriorDesign::model()->deleteAll('id in (' . implode(',', $shouldDelete) . ')');
