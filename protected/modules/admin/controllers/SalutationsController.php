...
{
    public $pluralTitle = 'Salutations';
    public $singleTitle = 'Salutations';
    public $cannotDelete = array();
    public function actionCreate(){
        try {
            $model = new Salutations('create');
            if (isset($_POST['Salutations'])) {
                $model->attributes = $_POST['Salutations'];
                if($model->save())
				{
    ...
    public function actionIndex() {
        try {
            $model=new Salutations('search');
            $model->unsetAttributes();  // clear any default values
            if(isset($_GET['Salutations']))
                $model->attributes=$_GET['Salutations'];
	...
	*/
	...
	{
		$deleteItems = $_POST['salutations-grid_c0'];
		$shouldDelete = array_diff($deleteItems, $this->cannotDelete);

		if (!empty($shouldDelete))
		{
						Salutations::model()->deleteAll('id in (' . implode(',', $shouldDelete) . ')');
