...
class MasterTenuresController extends AdminController
{
    public $pluralTitle = 'Master Field Tenures';
    public $singleTitle = 'Master Field Tenures';
    public $cannotDelete = array();
    public function actionCreate(){
        try {
            $model = new MasterTenures('create');
            if (isset($_POST['MasterTenures'])) {
                $model->attributes = $_POST['MasterTenures'];
                if($model->save())
				{
    ...
    public function actionIndex() {
        try {
            $model=new MasterTenures('search');
            $model->unsetAttributes();  // clear any default values
            if(isset($_GET['MasterTenures']))
                $model->attributes=$_GET['MasterTenures'];
	...
	*/
	...
	{
		$deleteItems = $_POST['product-service-category-grid_c0'];
		$shouldDelete = array_diff($deleteItems, $this->cannotDelete);

		if (!empty($shouldDelete))
		{
			MasterTenures::model()->deleteAll('id in (' . implode(',', $shouldDelete) . ')');
