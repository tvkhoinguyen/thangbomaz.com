...
class KhungBanner2Controller extends AdminController
{
    public $pluralTitle = 'KhungBanner2';
    public $singleTitle = 'KhungBanner2';
    public $cannotDelete = array();
    public function actionCreate(){
        try {
            $model = new KhungBanner2('create');
            if (isset($_POST['KhungBanner2'])) {
                $model->attributes = $_POST['KhungBanner2'];
                $model->type = 'khungbanner2';
                if($model->save())
				{ 	
    ...
    public function actionIndex() {
        try {
            $model=new KhungBanner2('search');
            $model->unsetAttributes();  // clear any default values
            if(isset($_GET['KhungBanner2']))
                $model->attributes=$_GET['KhungBanner2'];
	...
	*/
	...
	{
		$deleteItems = $_POST['khung-banner2-grid_c0'];
		$shouldDelete = array_diff($deleteItems, $this->cannotDelete);

		if (!empty($shouldDelete))
		{
			$deleteImages = Page::model()->findAll('id in (' . implode(',', $shouldDelete) . ')');if (!empty($deleteImages)){	foreach($deleteImages as $item){$item->removeImage(array('image'), true);}}			KhungBanner2::model()->deleteAll('id in (' . implode(',', $shouldDelete) . ')');
