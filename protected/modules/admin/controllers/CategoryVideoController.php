...
class CategoryVideoController extends AdminController
{
    public $pluralTitle = 'Category Video';
    public $singleTitle = 'Category Video';
    public $cannotDelete = array();
    public function actionCreate(){
        try {
            $model = new CategoryVideo('create');
            if (isset($_POST['CategoryVideo'])) {
                $model->attributes = $_POST['CategoryVideo'];
                if($model->save())
				{
    ...
    public function actionIndex() {
        try {
            $model=new CategoryVideo('search');
            $model->unsetAttributes();  // clear any default values
            if(isset($_GET['CategoryVideo']))
                $model->attributes=$_GET['CategoryVideo'];
	...
	*/
	...
	{
		$deleteItems = $_POST['category-video-grid_c0'];
		$shouldDelete = array_diff($deleteItems, $this->cannotDelete);

		if (!empty($shouldDelete))
		{
			CategoryVideo::model()->deleteAll('id in (' . implode(',', $shouldDelete) . ')');
