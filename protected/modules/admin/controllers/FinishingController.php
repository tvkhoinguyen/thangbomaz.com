...
class FinishingController extends AdminController {
    ...
    public function actionIndex() {
        try {
            $model = new Finishing('search');
            $model->unsetAttributes();  // clear any default values
            if (isset($_GET['Finishing']))
                $model->attributes = $_GET['Finishing'];
    ...
    public function actionDeleteAll() {
        $deleteItems = $_POST['fishsing-grid_c0'];
        $shouldDelete = array_diff($deleteItems, $this->cannotDelete);

        if (!empty($shouldDelete)) {
            Finishing::model()->deleteAll('id in (' . implode(',', $shouldDelete) . ')');
