...
class GallerydesignerController extends AdminController
{
    public $pluralTitle = 'Gallery Management';
...
/*    public function init(){
    ...
    */
    ...
    protected function beforeAction($action){
        if($action->id !='ajaxActivate' && $action->id !='ajaxDeactivate' ){
            ...
            $this->designerId = $model;   
            $this->baseControllerIndexUrl  = Yii::app()->createAbsoluteUrl('admin/gallerydesigner/index',array('designerId'=>$this->designerId->id));         
        }
    ...
    public function actionCreate(){
        try {
            $model = new InteriorDesignGallery('create');
            if (isset($_POST['InteriorDesignGallery'])) {
                $model->attributes = $_POST['InteriorDesignGallery'];
                if($model->save())
				{
    ...
    public function actionIndex() {
        try {
            $model=new InteriorDesignGallery('search');
            $model->unsetAttributes();  // clear any default values
            if(isset($_GET['InteriorDesignGallery']))
                $model->attributes=$_GET['InteriorDesignGallery'];
	...
	*/
	...
	{
		$deleteItems = $_POST['interior-design-gallery-grid_c0'];
		$shouldDelete = array_diff($deleteItems, $this->cannotDelete);

		if (!empty($shouldDelete))
		{
            if(is_array($shouldDelete) && count($shouldDelete)>0){
                $allData = InteriorDesignGallery::model()->findAllByAttributes(array("id"=>$shouldDelete));    
            ...
            //$deleteImages = Page::model()->findAll('id in (' . implode(',', $shouldDelete) . ')');if (!empty($deleteImages)){	foreach($deleteImages as $item){$item->removeImage(array('image'), true);}}			InteriorDesignGallery::model()->deleteAll('id in (' . implode(',', $shouldDelete) . ')');
		...
		if (!isset($_GET['ajax']))
			$this->redirect($this->redirect($this->baseControllerIndexUrl));
	}
	...
	*/
    ...
    public function loadModel($id){
		...
		$initMode = new InteriorDesignGallery();
