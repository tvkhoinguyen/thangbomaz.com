...
$('#clearsearch').click(function(){
	...
	$.fn.yiiGridView.update('request-grid', {data: data});
	return false;
});

$('.deleteall-button').click(function(){
        var atLeastOneIsChecked = $('input[name=\"request-grid_c0[]\"]:checked').length > 0;
        ...
        {
                document.getElementById('request-grid-bulk').action='" . Yii::app()->createAbsoluteUrl('admin/' . Yii::app()->controller->id  . '/deleteall') . "';
                document.getElementById('request-grid-bulk').submit();
        }
});
...
Yii::app()->clientScript->registerScript('ajaxupdate', "
    $('#request-grid a.ajaxupdate').live('click', function() {
        $.fn.yiiGridView.update('request-grid', {
            type: 'POST',
            url: $(this).attr('href'),
            success: function() {
		...
		<?php 
			...
			$columnArray = array_merge($columnArray, array(
				...
				array(
					...
					'value'=>'Salutations::model()->findByPk($data->title)->name',
				),
				'first_name',
				'last_name',
				'email',
				...
				array(
					...
					'htmlOptions' => array('style' => 'text-align:right;'),
					'value'=>'Request::$type_of_solution[$data->type_of_solution]',
				...
				array(
					'name'=>'attachment',
					'type'=>'raw',
					'value'=>' !empty($data->attachment) ? "<a href=\'". Yii::app()->createAbsoluteUrl("/")."/upload/request/".$data->id."/".$data->attachment."\' >download here</a>" : "" ',
					'htmlOptions' => array('style' => 'text-align:center;')
			   ),
				array(
					'name'=>'status',
