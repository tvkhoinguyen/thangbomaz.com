...
$('#clearsearch').click(function(){
	...
	$.fn.yiiGridView.update('unit-grid', {data: data});
	return false;
});

$('.deleteall-button').click(function(){
        var atLeastOneIsChecked = $('input[name=\"unit-grid_c0[]\"]:checked').length > 0;
        ...
        {
                document.getElementById('unit-grid-bulk').action='" . Yii::app()->createAbsoluteUrl('admin/' . Yii::app()->controller->id . '/deleteall') . "';
                document.getElementById('unit-grid-bulk').submit();
        }
});
...
Yii::app()->clientScript->registerScript('ajaxupdate', "
    $('#unit-grid a.ajaxupdate').live('click', function() {
        $.fn.yiiGridView.update('unit-grid', {
            type: 'POST',
            url: $(this).attr('href'),
            success: function() {
...
//        }
        $columnArray = array_merge($columnArray, array(
            ...
            array(
                'htmlOptions' => array('width' => '75px', 'style' => 'text-align:left;'),
                'header' => 'Actions',
                'class' => 'CButtonColumn',
                'template' => '{view} {update} {delete}',
                'buttons' => array(
                    'delete' => array('visible' => '$data->allowDelete()'),
                ), 
            ),
        ));
